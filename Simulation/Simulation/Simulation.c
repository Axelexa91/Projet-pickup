	...
	else
	{
		while (!feof(fic_rep))
		{
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				char mot[10];
				if (lire_champ_suivant(buffer, &idx, mot, 5, SEPARATEUR) == OK) //On vérifie que e nom est OK
				{
					idx++;							/* on saute le separateur */
					newSlot.ClientID = atoi(mot);
					if (lire_champ_suivant(buffer, &idx, mot, 5, SEPARATEUR) == OK) //On vérifie que le prnom est OK
					{
						idx++;
	...
	else
	{
		while (!feof(fic_rep))
		{
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				char mot[10];
				//On définit un enregistrement dans lequel on va enregistré le nom, le prénom et le numéro de téléphone lu sur la ligne enregistré dans le buffer
				if (lire_champ_suivant(buffer, &idx, mot, 5, SEPARATEUR) == OK)//Problème dans lire_champ_suivant
				{
					idx++;							/* on saute le separateur */
					tmp.ID = atoi(mot);
					if (lire_champ_suivant(buffer, &idx, tmp.Nom, CLIENT_NOM_MAX, SEPARATEUR) == OK)
					{
						idx++;
						if (lire_champ_suivant(buffer, &idx, tmp.Prenom, CLIENT_PRENOM_MAX, SEPARATEUR) == OK)
						{
							idx++;
							//Et on l'enregistre dans le repertoire (variable)
							if (lire_champ_suivant(buffer, &idx, tmp.Adresse, CLIENT_ADRESSE_MAX, SEPARATEUR) == OK)
							{
								idx++;
								if (lire_champ_suivant(buffer, &idx, tmp.Ville, CLIENT_VILLE_MAX, SEPARATEUR) == OK)
								{
									idx++;
									if (lire_champ_suivant(buffer, &idx, tmp.CodePostal, CLIENT_CodePostal_MAX, SEPARATEUR) == OK)
									{
										idx++;
										if (lire_champ_suivant(buffer, &idx, tmp.NumeroTel, CLIENT_TELEPHONE_MAX, SEPARATEUR) == OK)
										{
											idx++;
											if (lire_champ_suivant(buffer, &idx, tmp.MotDePasse, CLIENT_MotDePasse_MAX, SEPARATEUR) == OK)
											{
		...
		while (!feof(fic_rep))
		{
			...
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				if (lire_champ_suivant(buffer, &idx, mot, 5, SEPARATEUR) == OK)//Problème dans lire_champ_suivant
				{
					...
					if (lire_champ_suivant(buffer, &idx, mot, 4, SEPARATEUR) == OK)
					{
						idx++;
						tmp.ID_Station = atoi(mot);
						if (lire_champ_suivant(buffer, &idx, mot, CLIENT_MotDePasse_MAX, SEPARATEUR) == OK)
						{
...
/***********************************/
/*Tris des stations pour un client*/
/***********************************/
...
//Trouve le slot optimal (moins de perte de volume)
...
/*A FINIR*/
int ReservationSlot(ReserveSlotList *rep, VentesFile *ventes, RecupDistanceList *distancesList, ListClient *clientsliste, ListArticle * ListArticles, SlotDispoFile * slotdispos, char nom_fichier_reservation_slots[], char nom_fichier_ventes[])
{
	...
	for (int i = 0; i < ventes->Size; i++)
	{
		...
		if (trouver != ERROR)
		{
			...
			//Il faut réserver le slot dans la liste (il apparâit plusieurs fois)
	...
	//On sauvegarde dns le fichiers ventes les commandes à retrîter la prochaine fois
	SauvegarderVentesNonTraitee(VentesNonTraitees, nom_fichier_ventes);
...
//Fonction sauvegarder pour modifier fichier station pick up 
			...
			while (!feof(fic_rep))
			{
				if (fgets(buffer, long_max_rec, fic_rep) != NULL)
				{
					...
					idx = 0;								/* analyse depuis le debut de la ligne */
															// ajouter code implemention liste
					char mot[51];
					if (lire_champ_suivant(buffer, &idx, mot, 4, SEPARATEUR) == OK) //On vérifie que e nom est OK
					{
						idx++;							/* on saute le separateur */
						if (lire_champ_suivant(buffer, &idx, mot, 51, SEPARATEUR) == OK) //On vérifie que le prnom est OK
						{
...
int DispatchingPickUP()
{
	SlotDispoFile * listslotsdispos = newSlotDispoFile();
	OuvertureFichiersSlotsDispos(listslotsdispos, "EnsembleSlotsDisposParRegion.txt");
	ReserveSlotList * listreservationslots = newReserveSlotList();
	VentesFile *ventes = newVentesFile();
	ChargerVentes(ventes, "Ventes.txt");
	RecupDistanceList * distanceList = newRecupDistanceList();
	ChargerRecupDistance(distanceList, "Distances.txt");
	ListClient * clientsliste = newListClient();
	ChargerClient(clientsliste, "Clients.txt");
	ListArticle * ListArticles = newListArticle();
	ChargerCommande(ListArticles, "Commande.txt");
	ReservationSlot(listreservationslots, ventes, distanceList, clientsliste, ListArticles, listslotsdispos, "ReservationSlots.txt", "Ventes.txt");
	return(EXIT_SUCCESS);
}
	...
	else
	{
		while (!feof(fic_rep))
		{
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				char mot[10];
				//On définit un enregistrement dans lequel on va enregistré le nom, le prénom et le numéro de téléphone lu sur la ligne enregistré dans le buffer
				if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_IDArticle + 1, SEPARATEUR) == OK)//Problème dans lire_champ_suivant
				{
					idx++;							/* on saute le separateur */
					tmp.IDArticle = atoi(mot);
					if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_Article + 1, SEPARATEUR) == OK)
					{
						idx++;
						tmp.quantite = atoi(mot);
						if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_Dim + 1, SEPARATEUR) == OK)
						{
							idx++;
							tmp.largeur = atoi(mot);
							//Et on l'enregistre dans le repertoire (variable)
	...
	else
	{
		while (!feof(fic_rep))
		{
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				char mot[10];
				if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_ID_PICKUP + 1, SEPARATEUR) == OK) //On vérifie que e nom est OK
				{
					idx++;							/* on saute le separateur */
					newSlot.PickupStationID = atoi(mot);
					if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_ID_SLOT + 1, SEPARATEUR) == OK) //On vérifie que le prnom est OK
					{
						idx++;
						newSlot.SlotID = atoi(mot);
						if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_Dim + 1, SEPARATEUR) == OK) //On vérifie que le prnom est OK
						{
							idx++;
							...
							if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_Dim + 1, SEPARATEUR) == OK) //On vérifie que le prnom est OK
							{
								...
								if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_Dim + 1, SEPARATEUR) == OK) //On vérifie que le prnom est OK
								{
									...
									if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_Dim + 1, SEPARATEUR) == OK) //on verifie que le tel est OK
...
/********** Test *********/
int ResultatCapacite()
{
	RetourDisponibiliteSlot("Commande.txt", "Station001.txt", "SlotsDispos001.txt");
	RetourDisponibiliteSlot("Commande.txt", "Station002.txt", "SlotsDispos002.txt");
	return(EXIT_SUCCESS);
}
			...
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_ID_PICKUP + 1, SEPARATEUR) == OK) //On vérifie que e nom est OK
				{
					...
					if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_ID_SLOT + 1, SEPARATEUR) == OK) //On vérifie que le prnom est OK
					{
						...
						if (lire_champ_suivantPickUP(buffer, &idx, mot, 5, SEPARATEUR) == OK) //On vérifie que le prnom est OK
						{
							idx++;
							...
							if (lire_champ_suivantPickUP(buffer, &idx, mot, 5, SEPARATEUR) == OK) //On vérifie que le prnom est OK
							{
								...
								if (lire_champ_suivantPickUP(buffer, &idx, mot, 20, SEPARATEUR) == OK) //On vérifie que le prnom est OK
								{
									idx++;
									ptr.numero_tel = atoi(mot);
									if (lire_champ_suivantPickUP(buffer, &idx, mot, 5, SEPARATEUR) == OK) //on verifie que le tel est OK
										ptr.login = atoi(mot);
									num_rec++;
								}
							}
			...
			if (fgets(buffer, long_max_rec, fic_rep) != NULL)
			{
				...
				if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_ID_PICKUP + 1, SEPARATEUR) == OK) //On vérifie que e nom est OK
				{
					...
					if (lire_champ_suivantPickUP(buffer, &idx, mot, MAX_ID_SLOT + 1, SEPARATEUR) == OK) //On vérifie que le prnom est OK
					{
						idx++;
						ptr.ID_Slot = atoi(mot);
						if (lire_champ_suivantPickUP(buffer, &idx, mot, 21, SEPARATEUR) == OK) //On vérifie que le prnom est OK
						{
							idx++;
							ptr.Tel = atoi(mot);
							if (lire_champ_suivantPickUP(buffer, &idx, mot, 5, SEPARATEUR) == OK) //On vérifie que le prnom est OK
