...
/*Cette fonction est activée à chaque début de fuseaux des 4h, elle permet de créer un tableau dans lequel sera contenu toutes les commandes effectuées durant 4h.*/
...
/*Cette fonction est activée lors de la connexion du client, elle permet d'initialiser le panier dans lequel sera disposer ses commandes éventuels.*/
int init_panier(CommandeFile* Panier) {
	void* Ptr = (void*)malloc(COMMANDEFILE_MAX * sizeof(Commande));
	if (Ptr != NULL) {
